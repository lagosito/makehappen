...
class WPxmlImporter_inlineImages {
	...
	public function process_multi_images($data_array, $helperObj, $currentLimit, $impObj, $import_image_method, $imgLoc, $sampleURL) {
		...
		foreach($res as $key => $image_url) {
			foreach($image_url as $img_path) {
				...
				$imagelist = xmlscanDirectories($inlineimageDirpath);
				if(!$imagelist) {
...
//			$string = "[WPIMPINLINE:rr.jpg|size=20|image=20]";
		foreach($inlineimg_shortcodes as $shortkey => $shortcode){
			...
			$currentLoc = '';
                        if(!$imagelist) {
				$noimage = WP_CONST_ADVANCED_XML_IMP_DIR . "images/noimage.png";
				$oldWord = $shortcode;
			...
			if (@getimagesize($wp_media_path)) {
				...
				$get_inline_guid = str_replace(' ', '-', $get_inlineimage_val);
				 ...
				 $uploadedImage = $wp_upload_dir['path'] . '/' . $get_inline_guid;	
				//duplicate check
				global $wpdb;
...
function xmlscanDirectories($rootDir, $allData=array()) {
         ...
         foreach($dirContent as $key => $content) {
                ...
                if(!in_array($content, $invisibleFileNames)) {
                        ...
                        if(is_file($path) && is_readable($path)) {
                                ...
                                $allData = xmlscanDirectories($path, $allData);
                        }
                }
        }
        return $allData;
