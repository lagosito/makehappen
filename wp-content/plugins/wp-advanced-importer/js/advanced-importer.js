jQuery( document ).ready(function() {
		...
		document.getElementById('log').innerHTML = '<p style="margin:15px;color:red;">'+ wp_advanced_translate_importer.dboard_msg +'</p>';
		}
...
function import_xml() 
{
		...
		if (selectedpsindex == 6){
	                if (importer != 'custompost' && val1 == 'On' && post_status_msg == 'On' && val7 == 'On') {
                          ...
                          if (post_status_msg == 'Off')
                        	 ...
                        	 showMapMessages('error', 'Error: ' + error_msg + wp_advanced_translate_importer.req_msg);
                        	 return false;
               		  }
	...
	// validation starts
		...
		if(val1 == 'On' && val2 == 'On' && val3 == 'On') {
			...
			if (val3 == 'Off')
				error_msg += " role";
			showMapMessages('error','Error: ' + error_msg + wp_advanced_translate_importer.general_msg);
			return false;
		}
	}
	// validation ends
...
function checkextension(filename)
{
	...
	{
		alert(wp_advanced_translate_importer.file_validate);
...
function importRecordsbySettings(siteurl)
{
	...
	if(get_requested_count != '') {
		...
		document.getElementById('showMsg').style.display = "";
		document.getElementById('showMsg').innerHTML = '<p id="warning-msg" class="alert alert-warning">'+ wp_advanced_translate_importer.reqfield_msg +'</p>';	
		jQuery("#showMsg").fadeOut(10000);
		return false;
	}
	...
	if(get_log == '<p style="margin:15px;color:red;">NO LOGS YET NOW.</p>'){
		document.getElementById('log').innerHTML = '<p style="margin-left:10px;color:red;">'+ wp_advanced_translate_importer.import_prog +'</p>';
		document.getElementById('startbutton').disabled = true;
	}
...
setTimeout(function(){importRecordsbySettings()},0);
	...
	if(parseInt(tmpCnt) < parseInt(tot_no_of_records)-1)
		document.getElementById('log').innerHTML += "<p style='margin-left:10px;color:red;'>"+ wp_advanced_translate_importer.terminate_import +"</p>";
	document.getElementById('ajaxloader').style.display="none";
	document.getElementById('startbutton').style.display = "none";
	document.getElementById('terminatenow').style.display = "none";
...
function continueprocess() {
	...
	if (parseInt(tmpCnt) < parseInt(tot_no_of_records))
		document.getElementById('log').innerHTML += "<div style='margin-left:10px;color:green;'>"+ wp_advanced_translate_importer.continue_mport +"</div></br>";
	document.getElementById('ajaxloader').style.display = "";
...
{  
	...
	{  
		if(inputtxt == '')
			alert(wp_advanced_translate_importer.reqfield_msg);
		else
			alert(wp_advanced_translate_importer.validate_int);  
		return false;  
	}  
}
