...
class UploadHandler
{
                                      ...
                                      $type = null, $index = null, $content_range = null)
    {
        ...
        $rem_xml = explode(".xml", $name);
        $version_zero = explode("(", $rem_xml[0]);
	...
	if(in_array('xml' ,$file_name_arr)) {
        	if (!is_array($version_zero))
            		return $file_name_arr[0] . '-' . $current_action . ' .xml'; // Code modified by Fredrick Marks
        	else
            		return $file_name_arr[0] . '-' . $current_action . '-1.xml'; // Code modified by Fredrick Marks
        //Edited original file ends
	 }
       if(in_array('txt' ,$file_name_arr)) {
               if(!is_array($version_zero))
                       return $file_name_arr[0] . '_' .$current_action . ' .txt';
               else
                       return $file_name_arr[0] . '_' .$current_action . '-1.txt';
       }
    ...
    {
	$post_url = admin_url() . 'admin.php?page=' . WP_CONST_ADVANCED_XML_IMP_SLUG . '/index.php&__module=' . sanitize_text_field($_POST['curr_action']). '&step=uploadfile';
	$impCheckobj = CallWPAdvImporterObj::checkSecurity();
		if($impCheckobj != 'true')
		die($impCheckobj);
    ...
    protected function generate_response($content, $print_response = true)
    {
        if ($print_response) {
            ...
            $redirect = isset($_REQUEST['redirect']) ?
                sanitize_text_field($_REQUEST['redirect']) : null;
            if ($redirect) {
