 ...
 */
function snax_add_image_item_metabox( $post_type, $post ) {
	if ( ! snax_is_item( $post, 'image' ) ) {
		return;
	}
	...
	add_meta_box(
		'snax_image_item',
		__( 'Snax', 'snax' ),
		'snax_image_item_metabox',
		$post_type,
		'normal',
		'default'
	);

	do_action( 'snax_register_image_item_metabox' );
}
 ...
 * @param WP_Post $post         Post object.
 */
function snax_image_item_metabox( $post ) {
	// Secure the form with nonce field.
	wp_nonce_field(
		'snax_image_item',
		'snax_image_item_nonce'
	);
 ...
 * @return mixed
 */
function snax_save_image_item_metabox( $post_id ) {
	$nonce = filter_input( INPUT_POST, 'snax_image_item_nonce', FILTER_SANITIZE_STRING );
	...
	// Verify nonce.
	if ( ! check_admin_referer( 'snax_image_item', 'snax_image_item_nonce' ) ) {
	...
	if ( ! empty( $post_data ) ) {
		$values = snax_sanitize_image_item_metabox_data( $post_data );
	...
	do_action( 'snax_save_image_item_metabox', $post_id );
 ...
 * @return array
 */
function snax_sanitize_image_item_metabox_data( $data ) {
	$sanitized = array();
