 ...
 */
function snax_add_image_item( $post_id = 0, $item_args ) {
	...
	add_post_meta( $item_id, '_snax_item_format', 'image' );
	add_post_meta( $item_id, '_snax_parent_format', $parent_format );
	add_post_meta( $item_id, '_snax_origin', $origin );

	// IP.
	add_post_meta( $item_id, '_snax_author_ip', snax_get_ip_address() );
	...
	do_action( 'snax_item_added', $item_id, _x( 'image', 'Item type', 'snax' ), $origin );
...
function snax_add_embed_item( $post_id = 0, $item_args ) {
	...
	do_action( 'snax_item_added', $item_id, _x( 'embed', 'Item type', 'snax' ) );
...
function snax_delete_item( $item_id, $user_id = 0 ) {
	...
	$delete_media = apply_filters( 'snax_delete_media', true, $thumbnail_id );
...
function snax_change_item_comments_order( $query ) {
	if ( ! snax_is_item() ) {
		return;
	}

	// On a list?
	if ( is_single( snax_get_item_parent_id() ) ) {
		$query->query_vars['order']  = 'DESC';
	}
}

function snax_change_item_comments_limit( $args ) {
	if ( ! snax_is_item() ) {
		return $args;
	}
