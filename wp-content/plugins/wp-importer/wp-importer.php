...
Description: Import data from a CSV file using this light weight plugin.
Version: 1.2
Author: Raghunath
...
<div class="wrap">
	...
	<h2 style="color:green;text-align:center;"><strong>Pay one time use lifetime!!!!!</strong> Hurry up!!!!</h2>
	
	<p>We have released an add-on for <strong><a href="https://wordpress.org/plugins/wp-importer/" target="_blank">WP Importer</a></strong> which not only demonstrates the flexibility of <strong>WP Importer</strong>, but also adds some important features:</p>
	<ol>
	...
	foreach ($arry as $data) {
		...
		$user_id =get_current_user_id();
			...
			if(isset($data['custom_id']) && $data['custom_id']!='')
			{
				...
				AND ( (wp_postmeta.meta_key = '_wp_importer_unique_id' 
				AND wp_postmeta.meta_value = '$customId') || (wp_postmeta.meta_key = 'custom_id' 
...
function wp_importer_add_settings_link( $links ) {
  $settings_link = '<a href="options-general.php?page=wp-importer">' . __( 'Settings', 'wp-importer' ) . '</a>';
